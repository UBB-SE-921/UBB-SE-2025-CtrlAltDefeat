    ...
    public class ContractModelTests
    {
        ...
        private void SetupReaderWithColumns(params string[] columnNames)
        {
            for (int columnIndex = 0; columnIndex < columnNames.Length; columnIndex++)
            {
                ...
                mockReader.Setup(Database_reader => Database_reader.GetOrdinal(columnNames[capturedColumnIndex])).Returns(capturedColumnIndex);
            }
        }
        ...
        private void SetupReaderWithFullContractColumns()
        {
           ...
           // Setup column ordinals for contract
            ...
            {
                mockReader.Setup(reader => reader.GetOrdinal(column.Key)).Returns(column.Value);
            }
        }
    }
}